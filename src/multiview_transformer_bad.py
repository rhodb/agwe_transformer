...
from layer.transformer.transformer_old import TransformerEncoder_default
  ...
  def forward_view(self, batch, view):
    ...
    if view == "view1":
      if "starts" not in batch:
        emb, _ = self.net["view1"](batch["view1"], batch["view1_lens"])
      else:
        emb = []
        out, lens = self.net["view1"](batch["view1"], batch["view1_lens"])
        ...
        emb = torch.stack(emb)
    if view == "view2":
      emb, _ = self.net["view2"](batch["view2"], batch["view2_lens"])
